document.addEventListener('DOMContentLoaded', () => {
    ...
    addShortcutBtn.addEventListener('click', async () => {
        const { value: formValues } = await Swal.fire({
            ...
            html: `
                <input id="swal-name" class="swal2-input" placeholder="Shortcut Name">
                <input id="swal-url" class="swal2-input" placeholder="URL (e.g., https://example.com)">
                <input id="swal-icon" class="swal2-input" placeholder="Icon Class (e.g., fab fa-github)">
                <div class="icon-preview">
                    <p>Common icons: fab fa-github, fab fa-youtube, far fa-envelope, fab fa-linkedin</p>
                    <p>Find more icons at: <a href="https://fontawesome.com/icons" target="_blank">FontAwesome</a></p>
                </div>
            `,
            focusConfirm: false,
        ...
        if (formValues) {
            // Validate URL format
            if (!formValues.url.startsWith('http://') && !formValues.url.startsWith('https://')) {
                formValues.url = 'https://' + formValues.url;
            }
            
            shortcuts.push({
                name: formValues.name || 'New Shortcut',
                url: formValues.url || 'https://',
                icon: formValues.icon || 'fas fa-link'
            });
...
async function handleBackgroundUpload(file) {
    try {
        ...
        if (file.size > 5 * 1024 * 1024) {
            const compressed = await compressImage(file);
            if (compressed.size > 5 * 1024 * 1024) {
                throw new Error('File size is too large even after compression');
            }
            file = compressed;
        }
        ...
        const reader = new FileReader();
        reader.onload = (e) => {
            handleBackgroundChange('custom', e.target.result);
...
// Enhanced shortcuts management
function updateShortcut(index, updates) {
    try {
        ...
        handleError(error, 'Failed to update shortcut');
    }
}
...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        showToast('Welcome to Dark Search!');
    } catch (error) {
        handleError(error, 'Failed to initialize some features');
    } finally {
        hideLoading();
    }
});
...
function updateTimeWithSeconds() {
    ...
    requestAnimationFrame(updateTimeWithSeconds);
}
...
async function initializeApp() {
    ...
    try {
        ...
        Promise.all([
            ...
            console.warn('Non-critical resource loading failed:', error);
        });

        updateTimeWithSeconds();
        showToast('Welcome to Dark Search!');
    } catch (error) {
        handleError(error, 'Some features may be limited');
    } finally {
        hideLoading();
    }
}
...
async function handleBackgroundUpload(file, retryCount = 3) {
    for (let i = 0; i < retryCount; i++) {
        try {
            ...
            const compressed = file.size > 5 * 1024 * 1024 
            ...
            handleBackgroundChange('custom', reader.result);
            showToast('Background updated successfully');
            return;
        } catch (error) {
            if (i === retryCount - 1) {
